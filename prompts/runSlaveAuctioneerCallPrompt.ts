...
export const runSlaveAuctioneerCallPrompt = (kb: KnowledgeBase, item: AuctionSlave, callCount: number): string => {
    ...
    return `
Bạn là một Đấu Giá Sư chuyên điều hành các phiên đấu giá Nô Lệ. Đã 20 giây trôi qua mà không có ai trả giá thêm. Bạn cần phải hô giá để thúc đẩy phiên đấu giá.

**BỐI CẢNH PHIÊN ĐẤU GIÁ:**
- **"Vật phẩm":** Nô lệ "${item.name}" (Cảnh giới: ${item.realm}, Tư chất: ${item.tuChat}) (Ký gửi bởi: ${item.ownerId === 'player' ? playerName : 'Nhà Đấu Giá'})
- **Giá cao nhất hiện tại:** ${item.currentBid} ${currencyName}, người giữ giá là **${item.highestBidderId === 'player' ? playerName : (kb.discoveredNPCs.find(n => n.id === item.highestBidderId)?.name || kb.slaveAuctionState?.auctionNPCs.find(n => n.id === item.highestBidderId)?.name || 'Một người mua bí ẩn')}**.
...
1.  **Tạo Lời Hô Giá Kịch Tính:**
    *   Viết một lời hô giá kịch tính, tạo cảm giác gấp rút. Đây là cơ hội cuối cùng! Một trong các NPC có thể quyết định ra giá vào phút chót.
    *   Sử dụng tag **[MESSAGE: "Nội dung lời hô giá"]**.
...
[MESSAGE: "Vậy ${item.currentBid} ${currencyName}, ${callText}! Không còn ai sao? Một mỹ nhân sắp có chủ mới!"]
`;
};